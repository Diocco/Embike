...
// Controladores
import { actualizarUsuario, agregarUsuario, eliminarUsuario, modificarDeseado, verDeseados, verUsuarios, verUsuarioToken } from '../controllers/usuarios.js';
// Middlewares
import { validarCampos, validarJWT, validarRolJWT, validarIDJWT } from '../middlewares/index.js';
// Verificaciones con la base de datos
import { esRolValido, correoUnico, nombreUnico, usuarioExiste } from '../../database/usuariosVerificaciones.js';
...
check('password', 'La contraseña es debe tener entre 8 y 20 caracteres').isLength({ min: 8, max: 20 }), check('correo', 'El correo no es valido').isEmail(), // Verifica que el email sea valido
check('correo').custom(correoUnico), check('rol').custom(esRolValido), validarCampos, // Devuelve un error al usuario si algun check fallo
agregarUsuario); // Agrega un nuevo usuario a la base de datos
// Actualiza la lista de productos deseados del usuario
router.put('/listaDeseados/:idProducto', validarJWT, validarCampos, // Devuelve un error al usuario si algun check fallo
modificarDeseado);
// Devuelve la lista de productos deseados
router.get('/listaDeseados', validarJWT, validarCampos, // Devuelve un error al usuario si algun check fallo
verDeseados);
router.put('/:id', //Actualiza un usuario
check('id').custom(usuarioExiste), // Verifica existencia y validez del id
validarJWT, validarIDJWT, check('rol').optional().custom(esRolValido), // Si se manda un rol verifica que sea valido
...
validarJWT, validarCampos, verUsuarioToken);
router.delete('/:id', // Elimina el usuario con el id pasado como parametro
validarJWT, validarRolJWT('admin'), check('id').custom(usuarioExiste), // Verifica existencia y validez del id
validarCampos, // Devuelve un error al usuario si algun check fallo
eliminarUsuario);
